...
function main() {
    ...
    if (args['one-panel-mode']) {
        ...
        util.deprecate(noop, `cloudcmd --no-one-panel-mode: deprecated, use --no-one-file-panel instead`, 'DP0003')();
        config('oneFilePanel', false);
        config('onePanelMode', false);
    }
    
    if (typeof env.bool('one_panel_mode') === 'boolean') {
        util.deprecate(noop, `CLOUDCMD_ONE_PANEL_MODE deprecated, use CLOUDCMD_ONE_FILE_PANEL instead`, 'DP0002')();
